...
#include "myBackwardDualDdtScheme.H"
...
template<class Type>
scalar myBackwardDualDdtScheme<Type>::deltaT_() const
{
    return physDeltaT_;
}
...
template<class Type>
scalar myBackwardDualDdtScheme<Type>::deltaT0_() const
{
    return physDeltaT0_;
}
...
scalar myBackwardDualDdtScheme<Type>::deltaT0_(const GeoField& vf) const
{
...
template<class Type>
template<class GeoField>
scalar myBackwardDualDdtScheme<Type>::deltaT_(const GeoField& vfOld) const
{
    return physDeltaT_;
}
...
template<class Type>
template<class GeoField>
scalar myBackwardDualDdtScheme<Type>::deltaT0_
(
...
myBackwardDualDdtScheme<Type>::fvcDdt
(
...
//! ----------------------------this is the one ised in invicide part----------------
//! ----------------------------this is the one ised in invicide part----------------
//! ----------------------------this is the one ised in invicide part----------------
//! ----------------------------this is the one ised in invicide part----------------
template<class Type>
tmp<fvMatrix<Type> >
myBackwardDualDdtScheme<Type>::fvmDdt
(
    const GeometricField<Type, fvPatchField, volMesh>& vf
)
{
    ...
    // Info<< "mesh().time().value()= " << mesh().time().value() / deltaT<<endl;
    label stepI = mesh().time().timeIndex();
...
{
            Info << "Hello from 2nd fvmDdt"<<endl;
...
{
            Info << "Hello from 3rd fvmDdt"<<endl;
        ...
        fvm.source() = 
            ...
            (
                ...
                - coefft0*vf.oldTime().internalField()*rho.oldTime().internalField()*
                  mesh().V0()
                + coefft00*vf.oldTime().oldTime().internalField()*rho.oldTime().oldTime().internalField()*
                  mesh().V00()
            );
    }
...
template<class Type>
tmp<typename myBackwardDualDdtScheme<Type>::fluxFieldType>
myBackwardDualDdtScheme<Type>::fvcDdtPhiCorr
(
...
template<class Type>
tmp<typename myBackwardDualDdtScheme<Type>::fluxFieldType>
myBackwardDualDdtScheme<Type>::fvcDdtConsistentPhiCorr
(
...
template<class Type>
tmp<surfaceScalarField> myBackwardDualDdtScheme<Type>::meshPhi
(
    const GeometricField<Type, fvPatchField, volMesh>& vf
)
{
    // Info << "Hello from myBackwardDualDdtScheme<Type>::meshPhi"<<endl;
    scalar deltaT = deltaT_();
    scalar deltaT0 = deltaT0_(vf);
