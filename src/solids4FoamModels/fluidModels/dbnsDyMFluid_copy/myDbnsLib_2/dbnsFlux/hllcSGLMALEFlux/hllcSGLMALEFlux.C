...
void Foam::hllcSGLMALEFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    const scalar rhoERight = rhoRight*Cv*TRight + pinf + rhoRight*q + 0.5*rhoRight*magSqr(URight_star);
    ...
    // Compute qLeft and qRight (q_{l,r} = U_{l,r} \bullet n)
    const scalar qLeft = (ULeft_star & normalVector) - qMesh;
    const scalar qRight = (URight_star & normalVector) - qMesh;
