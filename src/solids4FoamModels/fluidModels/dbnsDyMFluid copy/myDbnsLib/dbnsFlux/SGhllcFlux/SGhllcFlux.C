...
#include "SGhllcFlux.H"
#include "addToRunTimeSelectionTable.H"
namespace Foam
{
    defineTypeNameAndDebug(SGhllcFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, SGhllcFlux, dictionary);
}
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::SGhllcFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    // Density
    const scalar rhoLeft = (pLeft + pInf)/(CvLeft*(gamma-1)*TLeft);
    const scalar rhoRight = (pRight + pInf)/(CvRight*(gamma-1)*TRight);
