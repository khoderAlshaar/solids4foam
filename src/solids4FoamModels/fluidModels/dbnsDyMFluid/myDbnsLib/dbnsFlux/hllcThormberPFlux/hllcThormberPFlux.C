...
#include "hllcThormberPFlux.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam
{
    defineTypeNameAndDebug(hllcThormberPFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, hllcThormberPFlux, dictionary);
}

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::hllcThormberPFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
void Foam::hllcThormberPFlux::evaluateFlux_Wall
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    // Local Mach numbers at left and right states (based on total velocity magnitude)
    const scalar ML = mag(ULeft) / (aLeft + VSMALL);
    const scalar MR = mag(URight_wall) / (aRight + VSMALL);
