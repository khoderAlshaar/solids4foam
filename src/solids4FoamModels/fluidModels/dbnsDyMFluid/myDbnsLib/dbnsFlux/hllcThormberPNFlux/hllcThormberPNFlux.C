...
#include "hllcThormberPNFlux.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam
{
    defineTypeNameAndDebug(hllcThormberPNFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, hllcThormberPNFlux, dictionary);
}

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::hllcThormberPNFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    const scalar ML = mag(ULeft& normalVector) / (aLeft + VSMALL);
    const scalar MR = mag(URight& normalVector) / (aRight + VSMALL);
    
    // Scaling factor: min(1, max(ML, MR)) as per Equation (6) in the paper
    const scalar Ma_local = max(ML, MR); 

    const scalar zeta_1 = min(1.0, Ma_local) ;

    // Apply pressure sensor
    const scalar fp = min(fp1Left,fp1Right);
    
    const scalar zeta = 1- (1-zeta_1) *fp ;
    ...
    // const vector URight_star = URight;//0.5*(ULeft + URight)  + zeta * 0.5*(URight - ULeft);
    // const vector ULeft_star = ULeft;//0.5*(ULeft + URight)   + zeta * 0.5*(ULeft - URight);
    ...
    const scalar rhoELeft = rhoLeft*(CvLeft*TLeft+0.5*magSqr(ULeft_star));
    const scalar rhoERight = rhoRight*(CvRight*TRight+0.5*magSqr(URight_star));
    ...
    const scalar qLeft = (ULeft_star & normalVector);
    const scalar qRight = (URight_star & normalVector);
    ...
    const scalar aTilde =
        Foam::sqrt(max(0 ,(kappaTilde - 1)*(HTilde - 0.5*magSqr(UTilde))));
...
void Foam::hllcThormberPNFlux::evaluateFlux_Wall
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    const vector URight_wall = ULeft - 2.0*(ULeft & normalVector)*normalVector;
    ...
    // Local Mach numbers at left and right states (based on total velocity magnitude)
    const scalar ML = mag(ULeft& normalVector) / (aLeft + VSMALL);
    const scalar MR = mag(URight_wall& normalVector) / (aRight + VSMALL);
    ...
    const vector URight_star = 0.5*(ULeft + URight_wall)  + zeta * 0.5*(URight_wall - ULeft);
    const vector ULeft_star = 0.5*(ULeft + URight_wall)   + zeta * 0.5*(ULeft - URight_wall);
    // const vector URight_star = URight_wall;// 0.5*(ULeft + URight_wall)  + zeta * 0.5*(URight_wall - ULeft);
    // const vector ULeft_star = ULeft;// 0.5*(ULeft + URight_wall)   + zeta * 0.5*(ULeft - URight_wall);
