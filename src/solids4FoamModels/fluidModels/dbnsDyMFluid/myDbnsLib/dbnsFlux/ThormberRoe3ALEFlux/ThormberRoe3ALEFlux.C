...
#include "ThormberRoe3ALEFlux.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam
{
    defineTypeNameAndDebug(ThormberRoe3ALEFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, ThormberRoe3ALEFlux, dictionary);
}

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::ThormberRoe3ALEFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
//     };
    ...
    const scalar cLeft = Foam::sqrt(max((kappaLeft*(pLeft))/rhoLeft,SMALL));
    const scalar cRight = Foam::sqrt(max((kappaRight*(pRight ))/rhoRight,SMALL));
    ...
    scalar eLeft_star = CvLeft*TLeft + 0.5*magSqr(ULeft_star);
    scalar eRight_star = CvRight*TRight + 0.5*magSqr(URight_star);
    ...
    const scalar hLeft_star = eLeft_star + pLeft/rhoLeft;
    const scalar hRight_star = eRight_star + pRight/rhoRight;
    ...
    const scalar hTilde = hLeft_star*wLeft + hRight_star*wRight;
    ...
    const scalar deltaContrV_star =  deltaContrV;
