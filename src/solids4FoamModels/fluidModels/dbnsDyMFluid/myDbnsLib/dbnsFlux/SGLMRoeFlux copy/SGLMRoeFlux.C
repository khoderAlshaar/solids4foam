...
#include "SGLMRoeFlux.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam
{
    defineTypeNameAndDebug(SGLMRoeFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, SGLMRoeFlux, dictionary);
}

void Foam::SGLMRoeFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    const scalar cLeft  = sqrt(max((gamma*(pLeft + pInf))/rhoLeft,SMALL));
    const scalar cRight = sqrt(max((gamma*(pRight + pInf))/rhoRight,SMALL));
    ...
    const scalar cTilde = sqrt(max((gamma - 1)*(hTilde - 0.5*qTildeSquare), SMALL));
