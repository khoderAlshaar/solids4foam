...
namespace Foam
{
    defineTypeNameAndDebug(hllcLMSGFlux, 0);
    addToRunTimeSelectionTable(dbnsFlux, hllcLMSGFlux, dictionary);
}

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::hllcLMSGFlux::evaluateFlux
(
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
  ...
  //  };
    ...
    //!
    const scalar aTilde =
        Foam::sqrt(max(0 ,(gamma - 1)*(HTilde - 0.5*magSqr(contrUTilde) - q)));
