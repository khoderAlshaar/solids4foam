...
#include "compressibleFluidModel.H"
#include "volFields.H"
#include "fv.H"
#include "fvc.H"
...
namespace Foam
{
    defineTypeNameAndDebug(compressibleFluidModel, 0);
    defineRunTimeSelectionTable(compressibleFluidModel, dictionary);
    addToRunTimeSelectionTable(physicsModel, compressibleFluidModel, physicsModel);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::compressibleFluidModel::makePisoControl() const
{
    if (!pisoPtr_.empty())
    {
        FatalErrorIn("void Foam::compressibleFluidModel::makePisoControl() const")
...
void Foam::compressibleFluidModel::makePimpleControl() const
{
    if (!pimplePtr_.empty())
    {
        FatalErrorIn("void Foam::compressibleFluidModel::makePimpleControl() const")
            << "pointer already set" << abort(FatalError);
    }
...
void Foam::compressibleFluidModel::updateRobinFsiInterface
(
...
void Foam::compressibleFluidModel::CourantNo
(
    scalar& CoNum,
    scalar& meanCoNum,
    scalar& velMag
...
void Foam::compressibleFluidModel::CourantNo() const
{
    scalar CoNum = 0.0;
...
#if FOAMEXTEND
void Foam::compressibleFluidModel::oversetCourantNo
(
    scalar& CoNum,
    scalar& meanCoNum,
    scalar& velMag
...
void Foam::compressibleFluidModel::oversetCourantNo() const
{
    scalar CoNum = 0.0;
    scalar meanCoNum = 0.0;
    scalar velMag = 0.0;
...
void Foam::compressibleFluidModel::continuityErrs()
{
...
#if FOAMEXTEND
void Foam::compressibleFluidModel::oversetContinuityErrs()
{
...
void Foam::compressibleFluidModel::boundPU
(
    volScalarField& p,
    volVectorField& U
) const
...
#ifdef OPENFOAM_COM
Foam::meshObjects::gravity Foam::compressibleFluidModel::readG() const
#else
Foam::uniformDimensionedVectorField Foam::compressibleFluidModel::readG() const
#endif
{
...
Foam::compressibleFluidModel::compressibleFluidModel
(
    const word& type,
    Time& runTime,
    const word& region,
...
Foam::pisoControl& Foam::compressibleFluidModel::piso()
{
    if (pisoPtr_.empty())
    {
        makePisoControl();
...
Foam::pimpleControl& Foam::compressibleFluidModel::pimple()
{
    if (pimplePtr_.empty())
    {
        makePimpleControl();
...
#if FOAMEXTEND
const Foam::oversetMesh& Foam::compressibleFluidModel::osMesh() const
{
    return oversetMesh::New(mesh());
}
#endif

Foam::tmp<Foam::vectorField> Foam::compressibleFluidModel::faceZoneViscousForce
(
...
Foam::tmp<Foam::scalarField> Foam::compressibleFluidModel::faceZonePressureForce
(
    const label interfaceI
) const
{
...
Foam::tmp<Foam::scalarField> Foam::compressibleFluidModel::faceZoneTemperature
(
    const label interfaceI
) const
{
...
Foam::tmp<Foam::scalarField> Foam::compressibleFluidModel::faceZoneHeatFlux
(
    const label interfaceI
) const
{
...
Foam::tmp<Foam::scalarField> Foam::compressibleFluidModel::faceZoneHeatTransferCoeff
(
    const label interfaceI
) const
{
...
void Foam::compressibleFluidModel::UisRequired()
{
#ifdef OPENFOAM_NOT_EXTEND
...
#endif
    {
        FatalErrorIn(type() + "::UisRequired()")
            << "This compressibleFluidModel requires the 'U' field to be specified!"
            << abort(FatalError);
    }
}


void Foam::compressibleFluidModel::pisRequired()
{
#ifdef OPENFOAM_NOT_EXTEND
...
#endif
    {
        FatalErrorIn(type() + "::pisRequired()")
            << "This compressibleFluidModel requires the 'p' field to be specified!"
            << abort(FatalError);
    }
}

void Foam::compressibleFluidModel::makeGlobalPatches(const wordList& patchNames) const
{
...
Foam::compressibleFluidModel::globalPatches() const
{
...
void Foam::compressibleFluidModel::clearGlobalPatches() const
{
    globalPatchesPtrList_.clear();
}


void Foam::compressibleFluidModel::setDeltaT(Time& runTime)
{
    if (adjustTimeStep_)
    {
...
Foam::autoPtr<Foam::compressibleFluidModel> Foam::compressibleFluidModel::New
(
    Time& runTime,
    const word& region
)
{
    ...
    const word modelType(props.lookup("compressibleFluidModel"));

    Info<< nl << "Selecting compressibleFluidModel " << modelType << endl;
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorIn
        (
            "compressibleFluidModel::New(Time&, const word&)"
        )   << "Unknown compressibleFluidModel type " << modelType
            << endl << endl
            << "Valid compressibleFluidModel types are :" << endl
    ...
    return autoPtr<compressibleFluidModel>(ctorPtr(runTime, region));
}


void Foam::compressibleFluidModel::writeFields(const Time& runTime)
{
    physicsModel::writeFields(runTime);
}


bool Foam::compressibleFluidModel::read()
{
    if (regIOobject::read())
    {
        fluidProperties_ = subDict(type() + "Coeffs");
...
void Foam::compressibleFluidModel::end()
{
